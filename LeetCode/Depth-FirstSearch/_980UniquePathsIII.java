    ...
    class Solution {
        ...
        public int dfs(int grid[][], int x, int y, int zero) {
            ...
            if (x < 0 || y < 0 || x >= grid.length || y >= grid[0].length || grid[x][y] == -1)
                return 0;
            if (grid[x][y] == 2) {
                return zero == -1 ? 1 : 0; // Why zero = -1, because in above example we have 9 zero's. So, when we
                                           // reach the final cell we are covering one cell extra then the zero count.
            ...
            int totalPaths = dfs(grid, x + 1, y, zero) + // calculating all the paths available in 4 directions
                    dfs(grid, x - 1, y, zero) + dfs(grid, x, y + 1, zero) + dfs(grid, x, y - 1, zero);
